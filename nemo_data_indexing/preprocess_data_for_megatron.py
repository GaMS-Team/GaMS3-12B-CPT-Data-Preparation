...
def get_tokenizer(args):
    if args.tokenizer_library == "huggingface":
        return HFTokenizer(args.tokenizer_type)
...
class Encoder(object):
    ...
    def encode(self, json_line):
        if self.args.tokenized_data:
            assert not self.args.text_file, "Pretokenized corpora should be in JSON format"
            assert len(self.args.json_keys) == 1, "Only one JSON key (coresponding to token values) should be submitted"

            data = json.loads(json_line)
            ids = {}
            key = self.args.json_keys[0]
            tokens = data[key]
            padding_size = self.args.max_seq_len - len(tokens)
            tokens += padding_size * [Encoder.tokenizer.eos_id]
            ids[key] = [tokens]

        elif not self.args.text_file:
            data = json.loads(json_line)
            ids = {}
...
def get_args():
    ...
    group.add_argument('--text-file', action='store_true', help='Use text file instead of json.')
    group.add_argument("--tokenized-data", action="store_true", help="Activate if input data is already tokenized.")
    ...
    group.add_argument('--max-seq-len', type=int, default=8192, help="Maximal number of tokens in output documents.")
